	...
	var Check = func(ctx *context.Context) {
		...
		if user_auth_type > 0 {
			...
			if CheckAccess(params) {
				...
				if uinfo.(m.AdminUser).Username == adminuser {
					return
				}
				...
				if user_auth_type == 1 {
					...
					accesslist, _ = GetAccessList(uinfo.(m.AdminUser).Id)
				}
...
//check login
func CheckLogin(username string, password string) (user m.AdminUser, err error) {
	user = m.GetAdminUserByUsername(username)
	if user.Id == 0 {
