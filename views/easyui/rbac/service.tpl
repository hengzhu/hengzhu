...
var faqtypes=$.parseJSON({{.faqtypes | stringsToJson}})
...
$(function(){
    ...
    $("#datagrid").datagrid({
        ...
        columns:[[
            {field:'Id',title:'ID'},
            {field:"Name",title:"问题名称",align:"center",editor:"text"},
            {field:"FaqType",title:"问题类型",formatter:function(value){
                    for(var i=0; i<faqtypes.length; i++){
                        if (faqtypes[i].Id == value){
                            return faqtypes[i].Desc;
                        } 
                    }
                    return value;
                },
            ...
            {field:"Desc",title:"问题解决方式",align:"center",editor:"text"},
...
function delrow(){
    $.messager.confirm('Confirm','你确定要删除?',function(r){
        if (r){
            ...
            vac.ajax(URL+'/DelFaq', {Id:row.Id}, 'POST', function(r){
                if(r.status){
                    $("#datagrid").datagrid('reload');
                }else{
                    vac.alert(r.info);
